...
function rehacerM() /// redibuja todo
{
 ...
 // boton1.dibujarB();
  ...
  if (ver_grafo == true)
  {
    fill(255, 0, 0);
    stroke(255, 0, 0);
    circle(map(xx[numex], minx, maxx, 600, 1200), map(yy_mod[numex], miny, maxy, 10, 510), 7);
    if (ruta.length>0) {
      fill(255, 255, 255);
      stroke(255, 255, 255);
      for (let i = 0; i< ruta.length; i++)
      {
        circle(map(xx[(ruta[i]-1)], minx, maxx, 600, 1200), map(yy_mod[(ruta[i]-1)], miny, maxy, 10, 510), 7);
      }
      fill(0);
      stroke(0);
    }
