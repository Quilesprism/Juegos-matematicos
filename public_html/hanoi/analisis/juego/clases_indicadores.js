...
function RHP(partida) 
{

  for (let i = 0; i < partida.length; i++)
  {
    let incremento_rama = 1;
    for(let j = 0; j < ruta_optima.length; j++)
    {

      if (partida1[i] == ruta_optima[j])
      {
        //incremento_rama = 0;
        metrica_RH =0;
      }
  ...
  for (let k =1; k< partida.length;k++)
  {
    if (partida[k]==partida[i]) 
    {
      incremento_rama=0;
    }
...
function MOCHP(partida)
{
  for (let i =0; i < sug_p1.length;i++){

    if (partida[i+1] == sug_p1[i])
    { 
     metrica_MOCH = metrica_MOCH +1;
   }
